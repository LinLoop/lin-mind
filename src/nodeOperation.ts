...
export const updateNodeStyle = function (object, history = true) {
  ...
  const origin = {
    ...
    fontSize: nodeEle.style.fontSize && nodeEle.style.fontSize,
  ...
  nodeEle.style.fontSize = object.style.fontSize ? object.style.fontSize + 'px' : ''
...
export const addChild = function (el: NodeElement, node: NodeObj, history = true) {
  ...
  if (history) {
    this.bus.fire('operation', { name: 'addChild', obj: newNodeObj })
  }
...
export const copyNode = function (node: NodeElement, to: NodeElement) {
  ...
  deepCloneObj.root = false // 修复根节点复制无法专注问题
